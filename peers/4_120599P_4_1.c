...
/*	struct pack reg;
	...
	int r =connect(p2,(struct sockaddr *)&serveraddr,sizeof(serveraddr));
	//int r = connect(p3,(struct sockaddr *)&paddr,sizeof(paddr));
	if(r < 0)
	errorrep("connect");
	int newfd = accept(lispeer,(struct sockaddr *)&remoteaddr,&addrlen);
	if(fdmax < newfd)
	fdmax = newfd;
	FD_SET(newfd,&read_fds);
	connections++;
	r = connect(p3,(struct sockaddr *)&paddr,sizeof(paddr));
	if(r < 0)
	errorrep("connect");
