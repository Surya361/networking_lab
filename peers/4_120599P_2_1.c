...
void main(int argc,char *argv[])
{
	...
	serveraddr.sin_port = htons(3302);
	...
	peeraddr.sin_port = htons(3301);

	//binding the peer to address
	bindsock(lpeer,peeraddr);
	
	//adding listener 
	FD_SET(lpeer,&master);
	FD_SET(lpeer,&read_fds);
	FD_SET(p2,&write_fds);
	FD_SET(p3,&write_fds);	
	FD_SET(p4,&write_fds);
		...
		for(i=0;i<=fdmax;i++)
		{
			if(FD_ISSET(i,&read_fds))
			{
				if(i == lpeer)
				{
					...
					newfd = accept(lpeer,(struct sockaddr *)&remoteaddr,&addrlen);
					if(newfd < 0)
					errorrep("accept");
					else
					{
			...
			for(i=0; i<=fdmax; i++)
			{
				if(FD_ISSET(i,&read_fds))	
				{
					if(i == lpeer)
					{
						...
						int newfd = accept(lpeer,(struct sockaddr *)&remote,&addrlen);
						if(newfd < 0)
						errorrep("accept");
						else
						{
